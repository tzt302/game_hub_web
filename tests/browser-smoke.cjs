...
(async()=>{
  ...
  const first=await page.locator('.tile').allTextContents();await page.locator('#board').dispatchEvent('pointerdown',{clientX:300,clientY:300,pointerId:1,pointerType:'touch'});await page.locator('#board').dispatchEvent('pointerup',{clientX:80,clientY:300,pointerId:1,pointerType:'touch'});await page.waitForTimeout(220);assert.equal(await page.locator('#undo').isEnabled(),true);assert.notDeepEqual(await page.locator('.tile').allTextContents(),[]);await page.locator('#undo').click();assert.equal(await page.locator('#message').textContent(),'已撤销上一步');
  await page.goto('http://127.0.0.1:8766/games/minesweeper/');
  assert.equal(await page.locator('.cell').count(),81);await page.locator('[data-mode="flag"]').click();await page.locator('.cell').first().click();assert.equal(await page.locator('.cell.flagged').count(),1);assert.equal(await page.locator('#remaining').textContent(),'9');
  await page.setViewportSize({width:1280,height:850});await page.goto('http://127.0.0.1:8766/games/2048/');await page.keyboard.press('ArrowLeft');await page.waitForTimeout(180);assert.equal(await page.locator('#undo').isEnabled(),true);await page.locator('[data-direction="right"]').click();
