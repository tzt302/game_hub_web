...
(async()=>{
  ...
  const first=await page.locator('.tile').allTextContents();for(const [start,end] of [[[300,300],[80,300]],[[80,300],[300,300]],[[200,300],[200,80]],[[200,80],[200,300]]]){await page.locator('#board').dispatchEvent('pointerdown',{clientX:start[0],clientY:start[1],pointerId:1,pointerType:'touch'});await page.locator('#board').dispatchEvent('pointerup',{clientX:end[0],clientY:end[1],pointerId:1,pointerType:'touch'});await page.waitForTimeout(320);if(await page.locator('#undo').isEnabled())break}assert.equal(await page.locator('#undo').isEnabled(),true);assert.notDeepEqual(await page.locator('.tile').allTextContents(),[]);await page.locator('#undo').click();assert.equal(await page.locator('#message').textContent(),'已撤销上一步');
